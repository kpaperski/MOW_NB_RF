...
dalc_r_f <- function (stud_merge_D) {
  ...
  print(confusionMatrix(data = RF_Day_pred, reference = test$Dalc))

  RF_ROC <- data.frame(prediction1 = RF_Day_prob[,1],
                        actual1 = as.numeric(RF_table_Day$Dalc == 1),
                        prediction2 = RF_Day_prob[,2],
                        actual2 = as.numeric(RF_table_Day$Dalc == 2),
                        prediction3 = RF_Day_prob[,3],
                        actual3 = as.numeric(RF_table_Day$Dalc == 3),
                        prediction4 = RF_Day_prob[,4],
                        actual4 = as.numeric(RF_table_Day$Dalc == 4),
                        prediction5 = RF_Day_prob[,5],
                        actual5 = as.numeric(RF_table_Day$Dalc == 5))
  #head(NB_ROC)
